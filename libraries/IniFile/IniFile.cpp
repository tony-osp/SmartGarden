...
bool IniFile::getValue_P(const char* section, const char* key,
			 char* buffer, size_t len, char *value, size_t vlen) const
{
		char	sectionName[21];
		char	keyName[21];
		
		strcpy_P(sectionName, section);
		strcpy_P(keyName, key);

		return getValue(sectionName, keyName, buffer, len, value, vlen);
}
			  ...
			  char* buffer, size_t len, int& val) const
{
		...
		return getValue( sectionName, keyName, buffer, len, val);
}
...
bool IniFile::getIPAddress_P(const char* section, const char* key,
		       char* buffer, size_t len, IPAddress& ip) const
{
		...
		return  getIPAddress(sectionName, keyName, buffer, len, ip);
}
